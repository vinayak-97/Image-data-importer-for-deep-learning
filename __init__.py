...
def img_data_importer(path):
    """
    Accepts only one parameter i.e path of main folder (mainfolder inside which all different categories folders are present), after generating
    the data, the output will be returned in tuple (X_train,y_train) which contains
    X_train(categories) and y_train(labels), How to execute is shown below
    X,y = img_data_importer(path of main folder), X holds the image data and y holds
    the labels
    """
    train_data = []
    X_train = []
    ...
    print("Generating & Shuffling the Data...")    
    for category in categories:
        ...
        for image in os.listdir(full_path):
            try:
                ...
                train_data.append([resized_img_arr,category])
